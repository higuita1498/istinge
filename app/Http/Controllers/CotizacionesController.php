...
class CotizacionesController extends Controller
{
    ...
    public function store(Request $request){
        if( Cotizacion::where('empresa',auth()->user()->empresa)->count() > 0){
                     //Tomamos el tiempo en el que se crea el registro
        Session::put('posttimer', Cotizacion::where('empresa',auth()->user()->empresa)->get()->last()->created_at);
        $sw = 1;
...
//Si el tiempo es de menos de 30 segundos mandamos al listado general
    if ($diasDiferencia <= 10) {
      ...
      return redirect('empresa/cotizaciones')->with('success', $mensaje);
  }
        }
