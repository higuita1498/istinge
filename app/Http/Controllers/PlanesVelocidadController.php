    ...
    public function planes(Request $request){
        ...
        return datatables()->eloquent($planes)
            ...
            ->editColumn('mikrotik', function (PlanesVelocidad $plan) {
                return "<a href=" . route('mikrotik.show', $plan->mikrotik()->id) . " target='_blank'>{$plan->mikrotik()->nombre}</div></a>, <a href=" . route('mikrotik.show', $plan->mikrotik1()->id) . " target='_blank'>{$plan->mikrotik1()->nombre}</div></a>, <a href=" . route('mikrotik.show', $plan->mikrotik2()->id) . " target='_blank'>{$plan->mikrotik2()->nombre}</div></a>";
                return ;
            })
