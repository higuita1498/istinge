  ...
  */
    ...
    if($empresa->estado_dian==0&&preg_match('/^([0-9a-zA-Z]{8}\-[0-9a-zA-Z]{4}\-[0-9a-zA-Z]{4}\-[0-9a-zA-Z]{4}\-[0-9a-zA-Z]{12})$/',$empresa->fe_resolucion)==1){
      ...
      for ($i=1; $i <=5; $i++) {
        ...
        if($res['statusCode']=='200'){
          $validos[]=(["statusCode"=>$res['statusCode'],
                       "trackId"=>$res['trackId'],
                       "uuid"=>$res['uuid'],
          ...
          $fallidos[]= (["statusCode"=>$res['statusCode'],
                       "trackId"=>$res['trackId'],
                       "uuid"=>$res['uuid'],
  ...
  public function generateCreditNote(){
    ...
    if($empresa->estado_dian==0&&preg_match('/^([0-9a-zA-Z]{8}\-[0-9a-zA-Z]{4}\-[0-9a-zA-Z]{4}\-[0-9a-zA-Z]{4}\-[0-9a-zA-Z]{12})$/',$empresa->fe_resolucion)==1){
      ...
      for ($i=1; $i <=5; $i++) {
        ...
        $xml=str_replace('--uuidfact--',Auth()->user()->empresa()->firstuuidfact(),$xml);
        $xml=str_replace('--horafecha--',$fechahora,$xml);
        $xml=str_replace('--nota--',$nota,$xml);
        $xml=str_replace('--codeciudad--',$municipios->codigo_completo,$xml);
        $xml=str_replace('--nameciudad--',$municipios->nombre,$xml);
  ...
  public function generateDebitNote(){
    ...
    if($empresa->estado_dian==0&&preg_match('/^([0-9a-zA-Z]{8}\-[0-9a-zA-Z]{4}\-[0-9a-zA-Z]{4}\-[0-9a-zA-Z]{4}\-[0-9a-zA-Z]{12})$/',$empresa->fe_resolucion)==1){
      ...
      if(count($validos)>0 && count($fallidos > 0))$empresa->estado_dian=1;
      $empresa->save();
   ...
   public function checkStatusDian()
  {
    ...
    */
    ...
    public function mikrotik_create(){
        $this->getAllPermissions(Auth::user()->id);
        view()->share(['title' => 'Nueva Mikrotik', 'icon' => 'fas fa-server']);
        view()->share(['seccion' => 'mikrotik', 'subseccion' => 'gestion_mikrotik']);
        return view('configuracion.mikrotik.create');
    }
    
    public function mikrotik_store(Request $request){
        view()->share(['seccion' => 'mikrotik', 'subseccion' => 'gestion_mikrotik']);
        $request->validate([
            'nombre' => 'required|max:200',
            'ip' => 'required|max:200',
            ...
            'puerto_api' => 'required|max:200',
            'segmento_ip' => 'required|max:200'
        ]);
        ...
        return redirect('empresa/configuracion/mikrotik')->with('success', $mensaje)->with('mikrotik_id', $mikrotik->id);
    }
    ...
    public function mikrotik_edit($id){
        ...
        if ($mikrotik) {
            ...
            view()->share(['seccion' => 'mikrotik', 'subseccion' => 'gestion_mikrotik']);
            return view('configuracion.mikrotik.edit')->with(compact('mikrotik'));
        }
        return redirect('empresa/configuracion/mikrotik')->with('danger', 'No existe un registro con ese id');
    }
    ...
    public function mikrotik_show($id){
        ...
        if ($mikrotik) {
            ...
            view()->share(['seccion' => 'mikrotik', 'subseccion' => 'gestion_mikrotik']);
            return view('configuracion.mikrotik.show')->with(compact('mikrotik'));
        }
        return redirect('empresa/configuracion/mikrotik')->with('danger', 'No existe un registro con ese id');
    }
    ...
    public function mikrotik_conectar($id){
        ...
        if ($mikrotik) {
            ...
            return redirect('empresa/configuracion/mikrotik')->with($type, $mensaje)->with('mikrotik_id', $mikrotik->id);
        }
        return redirect('empresa/configuracion/mikrotik')->with('danger', 'No existe un registro con ese id');
    }
    ...
    public function mikrotik_importar($id){
        ...
        if ($mikrotik) {
            ...
            if ($API->connect($mikrotik->ip,$mikrotik->usuario,$mikrotik->clave)) {
                ...
                $mensaje='SE HAN IMPORTADO TANTO CONTRATOS DESDE LA MIKROTIK '.$mikrotik->nombre;
                $type = 'success';
            } else {
                $mensaje='NO HEMOS PODIDO CONECTAR  LA MIKROTIK '.$mikrotik->nombre.', INTENTE NUEVAMENTE.';
                $type = 'danger';
            }
            return redirect('empresa/configuracion/mikrotik')->with($type, $mensaje)->with('mikrotik_id', $mikrotik->id);
        }
