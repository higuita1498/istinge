        ...
        <table class="table table-striped cliente">
          <tbody>
            <tr>
              ...
              <th width="60%">@if($factura->cliente) <a href="{{route('contactos.show',$factura->cliente()->id)}}" target="_blanck">{{$factura->cliente()->nombre}}</a> @else  {{$factura->cliente()->nombre}} @endif</th>
            ...
            <tr>
              <td>Teléfono</td>
              <th>@if($factura->cliente){{$factura->cliente()->telefono1}}@else {{$factura->cliente()->telefono}} @endif</th>
