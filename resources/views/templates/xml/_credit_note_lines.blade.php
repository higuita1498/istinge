@foreach ($items as $key => $item)
      <cac:CreditNoteLine>
        <cbc:ID>{{($key + 1)}}</cbc:ID>
        @if($item->descripcion != null)<cbc:Note>@if($FacturaVenta->tipo_operacion == 2){{"Contrato de servicios AIU por concepto de: "}}@endif {{$item->descripcion}}</cbc:Note>@endif
        
        <cbc:CreditedQuantity unitCode="94">{{number_format($item->cant, 2, '.', '')}}</cbc:CreditedQuantity>
