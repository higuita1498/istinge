@foreach ($items as $key => $item)
    <cac:InvoiceLine>
        <cbc:ID>{{($key + 1)}}</cbc:ID>
        @if($item->descripcion != null)<cbc:Note>$item->descripcion</cbc:Note>@endif
        <cbc:InvoicedQuantity unitCode="">{{number_format($item->cant, 2, '.', '')}}</cbc:InvoicedQuantity>
        ...
        @if($item->desc > 0)
        @include('templates.xmlAPI._allowance_charges', ['discountxproduct' => true])
        @endif
        {{-- TaxTotals line --}}
        @include('templates.xmlAPI._tax_totals', ['impuestoItem' => true])
        <cac:Item>
            <cbc:Description>{{$item->producto()}}</cbc:Description>
            <cac:StandardItemIdentification>
                <cbc:ID schemeID="" schemeName="" schemeAgencyID="">{{$item->ref}}</cbc:ID>
            </cac:StandardItemIdentification>
        </cac:Item>
        <cac:Price>
