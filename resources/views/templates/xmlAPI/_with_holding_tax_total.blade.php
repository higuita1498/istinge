...
if ($retencion->retencion()->tipo == 1) {
	...
	foreach ($FacturaVenta->totalAPI($FacturaVenta->empresa)->reten as $key => $reten) {
		if (isset($reten->total) && $reten->tipo == 1) {
			foreach ($FacturaVenta->totalAPI($FacturaVenta->empresa)->imp as $imp) {
...
else if($retencion->retencion()->tipo == 2)
{
	if ($FacturaVenta->totalAPI($FacturaVenta->empresa)->descuento > 0) {
		echo number_format($FacturaVenta->totalAPI($FacturaVenta->empresa)->subtotal - $FacturaVenta->totalAPI($FacturaVenta->empresa)->descuento, 2, '.','');
	}else
	{
		echo number_format($FacturaVenta->totalAPI($FacturaVenta->empresa)->subtotal,2,'.','');
	}
}
...
<cac:TaxScheme>
<cbc:ID>@if($retencion->retencion()->tipo == 1) {{ 5 }} @elseif($retencion->retencion()->tipo == 2) {{ 6 }} @else {{ "ZZ" }}@endif</cbc:ID>
