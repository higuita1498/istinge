...
@elseif(Auth::user()->rol >= 2 )
    ...
    {{--@if(isset($_SESSION['permisos']['427']))
        <li class="nav-item" id="wifi">
            <a  class="nav-link" href="{{route('wifi.index')}}">
                <i class="menu-icon fas fa-wifi"></i>
                <span class="menu-title">Contraseñas Wifi</span>
            </a>
        </li>
    @endif--}}
    
    @if(isset($_SESSION['permisos']['400']) || isset($_SESSION['permisos']['401']) || isset($_SESSION['permisos']['402']))
        <li class="nav-item" id="sincronizacion">
            <a class="nav-link" data-toggle="collapse" href="#ui-sincronizacion" aria-expanded="false" aria-controls="ui-sincronizacion">
                <i class="menu-icon fas fa-server"></i>
                <span class="menu-title">Servicio WISPRO</span>
                <i class="menu-arrow"></i>
            </a>
            <div class="collapse" id="ui-sincronizacion">
                <ul class="nav flex-column sub-menu">
                    <?php if(isset($_SESSION['permisos']['400'])){ ?>
                    <li class="nav-item">
                        <a class="nav-link" href="#" onclick="import_plans()">Sincronizar Planes</a>
                    </li>
                    <?php } ?>
                    <?php if(isset($_SESSION['permisos']['401'])){ ?>
                    <li class="nav-item">
                        <a class="nav-link" href="#" onclick="import_clients()">Sincronizar Clientes</a>
                    </li>
                    <?php } ?>
                    <?php if(isset($_SESSION['permisos']['402'])){ ?>
                    <li class="nav-item">
                        <a class="nav-link" href="#" onclick="import_contracts()">Sincronizar Contratos</a>
                    </li>
                    <?php } ?>
                    <?php if(isset($_SESSION['permisos']['421'])){ ?>
                    <li class="nav-item">
                        <a class="nav-link" href="#" onclick="import_bmus()">Sincronizar Servidores</a>
                    </li>
                    <?php } ?>
                    <li class="nav-item">
                        <a class="nav-link" href="#" onclick="import_all()">Sincronizar Todo</a>
                    </li>
                </ul>
            </div>
        </li>
    ...
    {{--@if(isset($_SESSION['permisos']['200']))
        <li class="nav-item" id="radicados">
            <a  class="nav-link" href="{{route('radicados.index')}}">
                <i class="menu-icon far fa-life-ring"></i>
                <span class="menu-title">Radicados</span>
            </a>
        </li>
    @endif--}}
    ...
    {{--@if(isset($_SESSION['permisos']['300']))
        <li class="nav-item" id="solicitudes">
...
@section('scripts')
  <script>
    function import_all(){
      $titleswal = "¿Desea realizar la sincronización completa con WISPRO?";
      $confirmswal = "Si, Sincronizar";

      Swal.fire({
        title: $titleswal,
        text: 'El proceso puede demorar varios minutos',
        type: 'warning',
    ...
    function import_plans(){
      $titleswal = "¿Desea realizar la sincronización de los planes con WISPRO?";
      $confirmswal = "Si, Sincronizar";
    ...
    function import_clients(){
      $titleswal = "¿Desea realizar la sincronización de los clientes con WISPRO?";
      $confirmswal = "Si, Sincronizar";
    ...
    function import_contracts(){
      $titleswal = "¿Desea realizar la sincronización de los contratos con WISPRO?";
      $confirmswal = "Si, Sincronizar";
    ...
    function import_bmus(){
      $titleswal = "¿Desea realizar la sincronización de los servidores con WISPRO?";
      $confirmswal = "Si, Sincronizar";
    ...
    function import_mikrotiks(){
        $.ajax({
            url: '/software/import_mikrotiks',
            method:'get',
            success: function(data){
              data=JSON.parse(data);
              console.log(data);
