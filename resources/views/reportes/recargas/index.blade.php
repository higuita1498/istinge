...
@section('content')
    ...
    <form id="form-reporte">
        <div class="row card-description">
            <div class="form-group col-md-4">
                ...
                <select class="form-control selectpicker" name="usuario" id="usuario" title="Seleccione" data-live-search="true" data-size="5">
                    @foreach($usuarios as $usuario)
                        <option value="{{$usuario->id}}" {{$request->usuario==$usuario->id?'selected':''}}>{{$usuario->nombres}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group col-md-4">
            ...
            <div class="col-md-12 table-responsive">
                ...
                @foreach($movimientos as $movimiento)
                    <tr>
                        ...
                        <td>{{ Auth::user()->empresa()->moneda.\App\Funcion::Parsear($movimiento->recarga) }}</td>
                    </tr>
                @endforeach
                </tbody>
                <tfoot class="thead-dark text-center">
                    <td colspan="3"></td>
                    <th>{{Auth::user()->empresa()->moneda}} {{App\Funcion::Parsear($totales)}}</th>
                </tfoot>
                </table>
    ...
    <input type="hidden" id="urlgenerar" value="{{route('reportes.recargas')}}">
    <input type="hidden" id="urlexportar" value="{{route('exportar.recargas')}}">
@endsection
