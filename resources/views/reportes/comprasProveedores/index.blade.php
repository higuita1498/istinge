...
@section('content')
            ...
            <div class="col-md-12 table-responsive">
                <table class="table table-striped table-hover " id="table-general">
                    ...
                    @foreach($proveedores as $proveedor)
                        <tr>
                            <td><a href="{{route('contactos.show', $proveedor['id'])}}" target="_blanck">{{$proveedor['nombre']}}</a> </td>
                            <td><a href="{{route('contactos.show', $proveedor['id'])}}" target="_blanck">{{$proveedor['rep']}}</a></td>
                            <td>{{Auth::user()->empresa()->moneda}}{{App\Funcion::Parsear($proveedor['subtotal'])}}</td>
                            <td>{{Auth::user()->empresa()->moneda}}{{App\Funcion::Parsear($proveedor['total'])}}</td>
                            <td></td>
                        </tr>
                    @endforeach
                    </tbody>
    ...
    <input type="hidden" id="urlgenerar" value="{{route('reportes.comprasProveedor')}}">
    <input type="hidden" id="urlexportar" value="{{route('exportar.ventasCliente')}}">
@endsection
