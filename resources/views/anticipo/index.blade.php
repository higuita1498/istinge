    ...
    @foreach($formasPago as $forma)
        <tr>
            <td>
                ...
                <td>
                <div clas="d-flex">
                    <a href="#"  onclick="delete_anticipo('{{$forma->id}}')"><i class="fas fa-times"></i></a>
                    <a href="#" onclick="edit_anticipo('{{$forma->id}}')" data-toggle="modal" data-target="#editForma" class="btn btn-icons"><i class="fas fa-edit"></i></a>
                </div>
            </td>
        </tr>
    @endforeach
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
          ...
          <form>
            ...
            <div class="row">
                <div class="form-group col-md-6">
                    ...
                    <select class="form-control form-control-sm selectpicker p-0" data-live-search="true" data-size="5" name="mediopago_edit" id="mediopago_edit" title="Medio de pago doc. electrónico" required="">
                        @foreach($mediosPago as $medio)
                            <option value="{{$medio->id}}">{{$medio->nombre}} - {{$medio->codigo}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col-md-6">
        ...
        <div class="modal-footer">
          ...
          <a type="button" onclick="update_anticipo()" class="btn btn-primary">Actualizar</a>
        </div>
      </div>
    </div>
  </div>
...
<script>
    ...
    function savePaymentMethod(){
        ...
        if(validate){
            //*Petición ajax.
            $.ajax({
            url: url +'/empresa/anticipo/store',
            ...
            success: function(pago){
                    ...
                    if(pago != null){   
                        ...
                        <tr>
                            ...
                            <td>
                                <div clas="d-flex">
                                    <a href="#" onclick="delete_anticipo('${pago.id}')"><i class="fas fa-times"></i></a>
                                    <a href="#" onclick="edit_anticipo('${pago.id}')" data-toggle="modal" data-target="#editForma" href=""><i class="fas fa-edit"></i></a>
                                </div>
                            </td>
                        </tr>
                        `);
...
function edit_anticipo(id) {

    var url = $("#url").val();
    $.ajax({
        url: url +'/empresa/anticipo/edit',
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        type: "GET",
        datatype: "json",
        data: {
...
function update_anticipo(){
    ...
    $.ajax({
        url: url +'/empresa/anticipo/update',
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        type: "POST",
        datatype: "json",
        data: {
...
function delete_anticipo(id){
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
        url: url +'/empresa/anticipo/delete',
        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        type: "POST",
        datatype: "json",
        data: {
