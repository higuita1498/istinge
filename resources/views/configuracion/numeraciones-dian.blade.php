...
<div class="row">
  ...
  <div class="col-md-5" style="text-align: left;">
    <a href="{{route('numeraciones_dian.create')}}" class="btn btn-primary btn-sm" ><i class="fas fa-plus"></i> Nueva numeración DIAN</a>
  </div>
</div>

<p style=" padding-left:2%">Indica el prefijo y número con el cual deben crearse tus facturas de venta DIAN. Puedes configurar múltiples numeraciones DIAN.</p>
	...
	<div class="row card-description">
        ...
        @foreach($numeraciones as $num)
          <tr @if($num->id==Session::get('numeracion_id')) class="active_table" @endif>
            ...
            <td>{{$num->prefijo}}</td>
            <td>{{$num->inicio}}</td>
            <td><a href="{{route('numeraciones_dian.edit',$num->id)}}" class="btn btn-outline-primary btn-icons"><i class="fas fa-edit"></i></a>
              @if($num->usado()==0)
