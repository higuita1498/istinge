...
@section('boton')
<a href="{{route('numeraciones_equivalente.create')}}" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> Nueva numeración</a>
@endsection
...
<div class="row">
  <div class="col-md-12" style="text-align: left; padding-left: 3%;">
    <p>Indica el prefijo y número con el cual deben crearse tus facturas de documento soporte.
    ...
    <table class="table table-striped table-hover" id="example">
      ...
      <tbody>
        ...
        <tr @if($num->id==Session::get('numeracion_id')) class="active_table" @endif>
          ...
          <td>{{$num->prefijo}}</td>
          <td>{{$num->inicio}}</td>
          <td><a href="{{route('numeraciones_equivalente.edit',$num->id)}}" class="btn btn-outline-primary btn-icons"><i class="fas fa-edit"></i></a>
            @if($num->usado()==0)
            <form action="{{ route('numeraciones_equivalente.destroy',$num->id) }}" method="post" class="delete_form" style="margin:  0;display: inline-block;" id="eliminar-num{{$num->id}}">
              @csrf
            ...
            @endif
            <form action="{{ route('numeraciones_equivalente.act_desc',$num->id) }}" method="POST" class="delete_form" style="margin:  0;display: inline-block;" id="act_desc-num{{$num->id}}">
              @csrf
            </form>
            @if($num->estado==1)
