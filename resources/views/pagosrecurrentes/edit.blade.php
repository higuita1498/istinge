...
@section('content')
	<form method="POST" action="{{ route('pagosrecurrentes.update', $gasto->id) }}" style="padding: 2% 3%;    " role="form" class="forms-sample" novalidate id="form-factura" >
      ...
      <div class="form-group row">
        ...
        {{--<div class="form-group row" title="Fecha en la que se crea el último comprobante">
          <label class="col-sm-4 col-form-label">Fecha Finalización</label>
          <div class="col-sm-8">
            <input type="text" class="form-control" id="vencimiento" name="vencimiento" value="@if($gasto->vencimiento){{date('d-m-Y', strtotime($gasto->vencimiento))}} @endif">
          </div>
        </div>--}}
