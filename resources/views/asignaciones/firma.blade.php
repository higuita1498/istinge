...
@section('content')
    ...
    <form method="POST" action="{{ route('asignaciones.store_firma', $contacto->referencia_asignacion) }}" style="padding: 2% 3%;" role="form" class="forms-sample" id="form-asignacion">
        ...
        @csrf
        @php
            $dominio = auth()->user()->empresa()->dominio;
        @endphp
        {{-- Imprime o debuggea las variables --}}

            <div>
                <embed src="{{ config('app.url') }}/software/empresa/asignaciones/{{ $contacto->id }}/imprimir" type="application/pdf" width="100%" height="800px" style="margin-bottom:50px;"/>
            </div>
