				...
				@foreach($mikrotiks as $mikrotik)
					<tr @if($mikrotik->id==Session::get('mikrotik_id')) class="active" @endif>
						...
						<td class="text-center">
						    ...
						    @if($mikrotik->status == 0)
						        <button title="Conectar Mikrotik" class="btn btn-outline-success btn-icons" type="submit" onclick="confirmar('conectar-{{$mikrotik->id}}', '¿Está seguro que desea conectar la Mikrotik {{$mikrotik->nombre}}?', '');"><i class="fas fa-plug"></i></button>
						        @else
						        <button title="Desconectar Mikrotik" class="btn btn-outline-danger btn-icons" type="submit" onclick="confirmar('conectar-{{$mikrotik->id}}', '¿Está seguro que desea desconectar la Mikrotik {{$mikrotik->nombre}}?', '');"><i class="fas fa-plug"></i></button>
						    @endif
						    ...
						    @if($mikrotik->status == 1 && $mikrotik->reglas == 0)
						    <button title="Aplicar Reglas de Corte" class="btn btn-outline-dark btn-icons" type="submit" onclick="confirmar('regla-{{$mikrotik->id}}', '¿Está seguro que desea aplicar las reglas de corte a esta Mikrotik {{$mikrotik->nombre}}?', '');"><i class="fas fa-plus"></i></button>
						    @endif
						    @endif
