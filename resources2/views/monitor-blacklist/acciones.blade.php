...
@if($blacklisted_count>0)
    <a href="{{route('monitor-blacklist.reporte', $id)}}" class="btn btn-outline-info btn-icons" title="Ver Reporte"><i class="fas fa-file-contract"></i></a>
@endif
@if(isset($session['758']))
    <button type="button" class="btn btn-outline-success btn-icons" type="submit" title="Verificar IP" onclick="confirmar('verificar{{$id}}', '¿Está seguro que desea verificar el blacklist?', 'Esto puede demorar unos minutos');"><i class="fas fa-check-double"></i></button>
@endif
