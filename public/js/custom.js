...
$(document).ready(function() {
  ...
  if ($('#form-empresa').length > 0) {
      ...
      $("#form-empresa").validate({language: 'es',
      rules: {
  ...
  if ($('#form-ajuste').length > 0) {
    $("#form-ajuste").validate({language: 'es',
      submitHandler: function(form) {
        $("#fecha").removeAttr("disabled");
        form.submit();
      }
  ...
  if ($('#form-factura').length > 0) {
    $('.precio').mask('000.000.000.000.000', {reverse: true});
    $('.nro').mask('000');
    $("#form-factura").validate({language: 'es',
      submitHandler: function(form) {
        $("#fecha").removeAttr("disabled");
    ...
    $('#fecha').change(function() {
      if ($('#vencimiento').length > 0) {vencimiento(); plazo(); }
  ...
  if ($('#form-ingreso').length > 0) {
    if ($('input:radio[name=tipo]:checked').val()==1) {
      if ($('#es_gastos').length==0) {
        if ($('#ingreso').length > 0) { factura_pendiente($('#ingreso').val()); }
        else{  factura_pendiente(); }
      }
      else{
        if ($('#ingreso').length > 0) { factura_proveedor_pendiente($('#ingreso').val()); }
        else{  factura_proveedor_pendiente(); }
    ...
    $('.precio').mask('000.000.000.000.000', {reverse: true});
    $('.nro').mask('000.000');
    $("#form-ingreso").validate({language: 'es',
      errorElement : 'div',
  ...
  if ($('#form-usuarios').length > 0) {
    $("#form-usuarios").validate({language: 'es',
      rules:{
        password: {
          required:true,
            minlength: 6
  ...
  if ($('#form-reporte').length > 0) {
    ...
    $('#desde').change(function() {
      $("#fechas").val('7'); $("#fechas").selectpicker('refresh');
    });
